...
public enum FreeParallaxPositionMode
{
    ...
    [Tooltip("Wrap and anchor to the bottom based on custom position")]
    WrapAnchorNode
}
...
public class FreeParallaxElement
{
    ...
    [Tooltip("Object used as the node in RepositionLogic PositionMode WrapAnchorNode")]
    ...
    public void SetupPosition(FreeParallax p, Camera c, int index)
    {
        ...
        for (int i = 0; i < GameObjects.Count; i++)
        {
            ...
            else
            {
                ...
                else if(RepositionLogic.PositionMode == FreeParallaxPositionMode.WrapAnchorNode)
                {
                    if (p.IsHorizontal)
                    {
                        FreeParallax.SetPosition(obj, r, offset, baseObjectPos.y);
                    }
                }
                else
                {
